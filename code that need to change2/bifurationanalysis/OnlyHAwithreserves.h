/*
  habswitchsimple.h -  Header file specifying the elementary life-history functions of
  ...
  Last modification: AMdR - Jun 21, 2021
*/
...
char *parameternames[PARAMETER_NR] = { "phi", "rho", "rho2", "q", "beta", "eta1", "eta2","eta3", "eoj","eoa", "ws","c"};

// Default values of all parameters
double parameter[PARAMETER_NR] = {0.1, 1.0, 1.0, 1.0, 1000.0, 1.0, 1.0, 1.0,0.2,0.2, 0.5,0.1};
...
#define RHO                       parameter[1]                                      // Relative carrying capacity in the harvested habitat
#define RHO2                      parameter[2]                                      // Relative carrying capacity in the marine reserve
#define Q                         parameter[3]                                      // Relation between the feeding rate on the two resources
#define BETA                      parameter[4]                                      // Fecundity scalar
#define ETA1                      parameter[5]                                      // Mortality in the breeding habitat nursery
#define ETA2                      parameter[6]                                      // Mortality in the harvested area
#define ETA3                      parameter[7]                                      // Mortality in the marine reserve
#define E0J                        parameter[8]                                      // Additikonal mortality juvenile in the harvested area
#define E0A                        parameter[9]                                      // Additikonal mortality adult in the harvested area

#define WS                        parameter[10]                                      // Length at habitat switch (15 cm)
#define C                        parameter[11]                                      // Length at habitat switch (15 cm)
/*
 *====================================================================================================================================
  ...
  if (lifestage[0] == 2)
    {
      if (BirthStateNr == 0)
        {
          fecundity[0][0] = (1 - C) * BETA*Q*F2;
          fecundity[0][1] = C * BETA*Q*F2;
        }
      else
        {
          fecundity[0][0] = (1 - C) * BETA*Q*F3;
          fecundity[0][1] = C * BETA*Q*F3;
        }      
    }
               ...
               double mortality[POPULATION_NR])
{
  ...
  else
  {
    if (BirthStateNr == 0)
   {
    if (lifestage[0] == 2)  mortality[0] = ETA2 + E0A;
    else mortality[0] = ETA2 + E0J;
    }
    else
      mortality[0] = ETA3;
  }
