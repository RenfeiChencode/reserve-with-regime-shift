...
#define ETSJ                       parameter[4]                                      // Scaled fishing mortality rate of juveniles in hab 2 harvested area
#define ETSA                      parameter[5]                                     // Scaled fishing mortality rate of adults in hab 2 harvested area
...

{
    ...
    for(i=0; i<cohort_no[CONS]; i++)
    {
        ...
        if (issmolted(i))
        {
            ...
            if (ismature(i)) popgrad[CONS][i][number] = ETA2 + ETSA;
            else popgrad[CONS][i][number] = ETA2 + ETSJ;
