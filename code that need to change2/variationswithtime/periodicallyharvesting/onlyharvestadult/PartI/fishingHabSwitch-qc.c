...
#define EETS                      parameter[5]                                      // Scaled escapement rate large consumers

#define WS                        parameter[6]                                      // Default switch size (scaled)
#define Q                         parameter[7]                                      // Attack rate ratio on resource 1 and 2
#define BETA                      parameter[8]                                      // Scaled fecundity parameter

#define HERITABILITY              parameter[9]                                      // Heritability of trait value (0.3)
#define GENETICVARIANCE           parameter[10]                                      // Half-width of the truncated normal distribution (0.25)
...

{
    ...
    for(i=0; i<cohort_no[CONS]; i++)
    {
        ...
        if (issmolted(i))
        {
            ...
            if (ismature(i))
            {
                adults   += EETS*popIDcard[CONS][i][IDnumber];
                adultws  += EETS*popIDcard[CONS][i][IDsmoltsize]*popIDcard[CONS][i][IDnumber];
            }
            else
 ...
 * [14]    16: Total number of fisheries yields
 * [15]    17: Total biomass of fisheries yields
 */

{
    ...
    double          yieldnumadults = 0.0, yieldbioadults = 0.0;
    ...
    for(i=0; i<cohort_no[CONS]; i++)
    {
        ...
        if (issmolted(i))
        {
            ...
            {
                adults      += EETS*popIDcard[CONS][i][IDnumber];
                adultsbio   += EETS*pop[CONS][i][size]*popIDcard[CONS][i][IDnumber];
                yieldnumadults      += ETS*popIDcard[CONS][i][IDnumber];
                yieldbioadults      += ETS*pop[CONS][i][size]*popIDcard[CONS][i][IDnumber];
    ...
    output[outnr++] = yieldnumadults;
    output[outnr++] = yieldbioadults;    
    return;
}
